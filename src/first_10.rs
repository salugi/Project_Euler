pub fn project_euler_01(){
    let mut sum = 0;
    (0..1000).for_each(|x|{if x%3==0||x%5==0 {sum += x}});
    println!("{:?}", sum);
}
pub fn project_euler_02(){
    let mut temp1 = 0;
    let mut temp2 = 1;
    let mut sum =0;
    loop {
...
pub fn project_euler_04(){
    let smoke = 998001;
    let mut palindrome = 0;
    let mut should_break = false;
    for x in (10000..smoke).rev() {
        let forwards = x.to_string();
...
pub fn project_euler_08(){
    ...
    for start in 0..num_str.len(){
        let stop =
            if start+13 > num_str.len() {
                let leftover = num_str.len() - start;
                start+leftover
            }else{
                start+13
            };

        let mut subslice = &num_str[start..stop];
        let mut temp :i64 = 1;
